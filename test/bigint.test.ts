...
import NativeBN from '../src/native'

describe('BigInteger', () => {
  it('should umod', () => {
    const sBigInt = '-7'
    const bn = new BN(sBigInt)
    const nativeBN = new NativeBN(sBigInt)
    const rBN = bn.umod(new BN(3))
    const rNative = nativeBN.umod(new NativeBN(3))
  ...
  it('should mod', () => {
    ...
    const rNative = nativeBN.mod(new NativeBN(3))
    expect(rBN.toString()).toEqual(rNative.toString())
  ...
  it('should convert string to bigint', () => {
    ...
    let nativeBN = new NativeBN(sBigInt)
    expect(bn.toString()).toBe('10')
    expect(nativeBN.toString()).toBe('10')
    sBigInt = '0xF0F0F0'
    bn = new BN(sBigInt)
    nativeBN = new NativeBN(sBigInt)
